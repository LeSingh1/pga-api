...
![CI](https://github.com/LeSingh1/pga-api/actions/workflows/test.yml/badge.svg) ![license](https://img.shields.io/badge/license-MIT-blue.svg) ![node](https://img.shields.io/badge/node-%E2%89%A518-brightgreen.svg) ![dependencies](https://img.shields.io/badge/dependencies-0-success.svg)

Unofficial Node client for ESPN's PGA golf data. ESPN serves PGA golf under
`golf/pga`, and this wraps those endpoints with retry/timeout
transport and parsers that return clean data.

Part of the [espn-api](https://github.com/LeSingh1/espn-api) family. The parent
is one client for every sport. This one is just PGA golf, in the style of
swar/nba_api.
...
cd pga-api
node examples/scoreboard.js   # PGA golf events
npm test
```
...
PGA golf is an individual sport, so `teams`, `roster` and `standings` mostly do not apply. Use `scoreboard`, `athlete`, `gamelog`, `splits`, `news`, `summary`, `odds`, and the parsed `scoreboardClean` / `gamelogClean`.

## Daily data

A scheduled GitHub Action runs once a day, pulls that day's PGA golf games from
