...
if (games.length === 0) {
  console.log(`${ymd}: no PGA golf games today; leaving snapshot unchanged`);
  process.exit(0);
}
mkdirSync("data", { recursive: true });
writeFileSync(
  "data/latest.json",
  JSON.stringify({ sport: "golf", league: "pga", date: ymd, count: games.length, games }, null, 2) + "\n",
);
console.log(`${ymd}: wrote ${games.length} PGA golf games -> data/latest.json`);
